        ...
        ws.current.onmessage = (event) => {
          try {
            ...
            if (data.type === 'keyword_detected' && role === 'magician') {
              if (data.keyword === 'start' && callbacks.onStartKeyword) {
                callbacks.onStartKeyword();
              } else if (data.keyword === 'end' && callbacks.onEndKeyword) {
                callbacks.onEndKeyword();
              }
            }

            // Real-time transcript from backend
            if (data.type === 'transcript' && role === 'magician') {
              if (data.text && callbacks.onTranscript) {
...
function App() {
  ...
  const wsCallbacks = {
    ...
    onTranscript: (text) => {
      console.log("📝 Transcript from backend:", text);
      setTranscript(text);
      if (magicActiveRef.current) {
  ...
  useEffect(() => {
    if (isMobile || role !== 'magician' || !speechTranscript || isProcessingRef.current) return;
  ...
  const finalizeMagicSession = async () => {
    ...
    // On mobile, don't stop recording, just send finalize signal
    if (!isMobile) {
      await stopAudioRecording();
      resetTranscript();
    }
  ...
  // --- Mobile: Continuous Recording with Streaming ---
  ...
  }, [role, ws, listening, startListening, isMobile]);
  ...
  // --- Mobile: Keyword Listening Toggle ---
  const handleMobileListeningToggle = async () => {
    if (isListeningForKeywords) {
      stopMobileKeywordListening();
    } else {
      await startMobileKeywordListening();
    }
  };

  // --- Desktop: Manual Mic Toggle ---
          ...
          <div className="mobile-info" style={{ 
            ...
            <p style={{ margin: 0, fontSize: '14px' }}>
              {isListeningForKeywords ? (
                ...
                <>📱 <strong>Mobile Mode:</strong> Click button to start listening for magic keywords. Backend will detect them automatically.</>
              )}
            </p>
          </div>
        )}
